...
Run once at the start of the project. It is seeded (random.seed(42)), so every team
member and every team will generate byte-for-byte identical CSVs.
"""
import csv
import math
import os
...
with open(os.path.join(DATA_DIR, "utilities.csv"), "w", newline="") as f:
    w = csv.writer(f)
    w.writerow(["Utility ID", "Name", "Alias", "Code", "Type", "Country", "Active"])
    w.writerows(utilities)

with open(os.path.join(DATA_DIR, "substations.csv"), "w", newline="") as f:
    w = csv.writer(f)
